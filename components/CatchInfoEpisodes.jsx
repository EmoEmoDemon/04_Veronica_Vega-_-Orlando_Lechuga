        ...
        <div key={season}>
          ...
          <table>
            ...
            {groupedEpisodes[season].map((episode) => (
              <tr key={episode.id}>
                <td className="episode-info1">S{episode.season} x Ep{episode.number}</td>
                <td className="episode-info2">{episode.name}</td>
                <td className="episode-info3" dangerouslySetInnerHTML={{ __html: episode.summary }} />
              </tr>
              ))}
            </tbody>
          </table>
